...
| -------------------- | ------------------------------------------ | -------------------------- |
| `homepage_cta_clicked`| Homepage CTA button clicked                | ctaName, location          |
| `login_started`      | OAuth login button clicked                 | provider                   |
| `login_completed`    | OAuth login successfully completed         | userId, provider           |
...
| `OPENROUTER_API_KEY`            | agent/matcher.ts — GPT-4o scoring via OpenRouter (replaces OPENAI_API_KEY) |
